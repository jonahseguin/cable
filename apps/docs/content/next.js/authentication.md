...
[removed] requires users to be authenticated before they can subscribe to channels.

In a Next.js app, you handle authentication using the `with[removed]` utility.

---

## How it works

When a client connects to [removed], it needs a **signed token** that describes which channels it should have access to.
...
[removed] takes care of signing the token and returning it to the client automatically.
...
[removed] enforces strict channel-based access control.
