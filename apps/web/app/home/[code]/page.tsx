import { HomeGithubJoinButton } from '@/components/home/home-github-join-button';
import { HomeRecentUsers } from '@/components/home/home-recent-users';
import HomeWaitlistProvider from '@/components/home/home-waitlist-provider';
import {
  allowLoginFlag,
...
import { Meteors } from '@[removed]/ui/components/magicui/meteors';
...
import { MotionCircle, MotionDiv, MotionPath } from '../../../components/home/motion-wrapper';
import { NavbarClient } from '../../../components/home/navbar-client';
import { SignupForm } from '../../../components/home/signup-form';
import { TabsClient } from '../../../components/home/tabs-client';
...
const codeExampleSchema = `export const channels = [removed].channels({
  'chat.{roomId}.messages': [removed].channels.rich({
    schema: z.object({
      message: z.string(),
      sender: z.string(),
      timestamp: z.number(),
...
const codeExampleClient = `// Get real-time messages with type safety
const { data: messages } = useChannel(
  channels.chat.messages.for({ roomId: currentRoom })
);
...
const { self, others, update } = usePresence(
  channels.chat.messages.for({ roomId: currentRoom })
);
...
// Type safety code samples
const typeDefinitionCode = `// Define once in your schema
channels.define('chat.{roomId}.messages', {
  schema: z.object({
    message: z.string(),
    sender: z.string(),
  }),
...
const typeErrorCode = `// TypeScript catches errors
...
// Presence code example
const presenceCode = `function Document() {
...
// History code example
const historyCode = `// Access message history with a single hook
const { history } = useHistory(channel);
// Messages available immediately on join`;

// Authentication code example
const authCode = `authorize: (conn) => {
...
const testimonials = [
  {
    quote:
      '[removed] changed how we build real-time features. What used to take weeks now takes hours, and the type safety is a game-changer for our team.',
    author: 'Sarah Chen',
    role: 'CTO at Streamline',
    avatar: 'https://avatar.vercel.sh/sarah-chen.png',
  },
  {
    quote:
      'The developer experience is incredible. Our engineers were able to add real-time collaboration features in a single sprint with zero WebSocket headaches.',
    author: 'Mark Johnson',
    role: 'Lead Engineer at DevTools',
    avatar: 'https://avatar.vercel.sh/mark-johnson.png',
  },
  {
    quote:
      'We migrated from a custom WebSocket solution that was becoming a maintenance nightmare. [removed] not only simplified our codebase but significantly improved performance.',
    author: 'Jessica Lee',
    role: 'VP Engineering at Collab',
    avatar: 'https://avatar.vercel.sh/jessica-lee.png',
  },
];
...
export default async function HomePage({ params }: { params: Params }) {
  ...
  return (
    <HomeWaitlistProvider>
      <div id="landing-page" className="dark relative flex min-h-screen flex-col antialiased">
        <div className="absolute z-0 h-screen w-screen">
          <Meteors number={20} minDuration={4} maxDuration={12} />
        </div>
        {/* Modern, minimal navbar with frosted glass effect */}
        <header className="bg-background/10 z-99 sticky top-0 backdrop-blur-md">
          <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6 sm:px-8">
            <div className="flex items-center gap-10">
              <Link href="/" className="flex items-center gap-2">
                <span className="text-foreground text-2xl font-semibold tracking-tight">
                  sock<span className="text-primary">8</span>
                </span>
              </Link>

              <nav className="hidden space-x-8 md:block"></nav>
            </div>

            <NavbarClient allowLogin={allowLogin} />
          </div>
        </header>

        {/* Hero Section - Refined, minimal aesthetic */}
        <section className="relative flex min-h-screen flex-col justify-center overflow-hidden">
          {/* <div
            className="absolute inset-0 opacity-[0.03]"
            style={{
              backgroundImage:
                "url(\"data:image/svg+xml,%3Csvg width='40' height='40' viewBox='0 0 40 40' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='%23000000' fill-opacity='1' fill-rule='evenodd'%3E%3Ccircle cx='20' cy='20' r='0.5'/%3E%3C/g%3E%3C/svg%3E\")",
              backgroundSize: '40px 40px',
            }}
          ></div> */}

          <div className="relative mx-auto flex max-w-7xl flex-1 flex-col justify-center px-6 sm:px-8">
            <div className="mx-auto">
              <div className="bg-primary/10 text-primary cursor-events-none mb-6 inline-flex select-none items-center gap-2 rounded-full border px-3 py-1 text-sm font-medium">
                <Unplug className="h-4 w-4" strokeWidth={2} />
                <span>Coming Soon</span>
              </div>
            </div>
            <div className="mx-auto max-w-3xl text-center">
              <h1 className="animate-in fade-in text-3xl font-bold tracking-tight duration-300 sm:text-4xl lg:text-7xl">
                <span className="whitespace-nowrap">
                  <span className="text-primary">Type-safe</span> real-time
                </span>
                <br />
                for modern applications
              </h1>
              <p className="animate-in fade-in slide-in-from-top-2 text-foreground/70 mx-auto mt-6 max-w-2xl text-lg">
                [removed] provides end-to-end type-safe WebSockets with real-time presence, history, and
                authentication. Build collaborative experiences without the complexity.
              </p>
              {waitlistEnabled && <SignupForm />}
            </div>

            <HomeRecentUsers
              displayWaitlistCount={displayWaitlistCount}
              displayWaitlistRecentUsers={displayWailistRecentUsers}
            />
          </div>
        </section>

        {/* Code Example Section - Merged with Type Safety Feature */}
        <section className="bg-card/20 border-border/20 relative border-y py-20 md:py-28">
          <div
            ...
            <div className="grid grid-cols-1 items-center gap-16 lg:grid-cols-2">
              <div>
                ...
                <p className="text-foreground/70 mt-6 text-lg leading-relaxed">
                  Build real-time features without the headaches of WebSockets. Our SDK provides
                  end-to-end type safety from schema definition to client interactions.
                </p>

                <ul className="mt-8 space-y-4">
                  <li className="flex items-start gap-3">
                    <Check className="text-primary mt-1 h-4 w-4 shrink-0" strokeWidth={2} />
              ...
              <TabsClient
                schemaContent={
                  <CodeBlock code={codeExampleSchema} language="typescript" theme="dark" />
                }
                clientContent={
                  <CodeBlock code={codeExampleClient} language="typescript" theme="dark" />
                }
                typeErrorContent={
                  <CodeBlock code={typeErrorCode} language="typescript" theme="dark" />
                }
              />
            </div>
          </div>
        ...
        {/* Features Section - Clean, modern aesthetic */}
        <section className="bg-card/20 relative py-20 md:py-28">
          <div
            className="absolute inset-0 opacity-[0.02]"
            style={{
              backgroundImage:
          ...
          <div className="mx-auto max-w-7xl px-6 sm:px-8">
            <div className="mx-auto mb-24 max-w-3xl text-center">
              ...
              <p className="text-foreground/70 mt-6 text-lg">
                Everything you need to build modern real-time applications with type safety at the
                core.
              </p>
            </div>

            {/* Parameterized Channels Feature - Refined visual design */}
            <div className="mb-24 md:mb-36 lg:mb-40">
              <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2">
                <div className="order-2 lg:order-1">
                  <div className="relative mx-auto max-w-md">
                    <div className="to-primary/5 absolute -inset-px rounded-lg bg-gradient-to-r from-blue-500/10 opacity-50 blur-md"></div>
                    <div className="bg-card/90 relative rounded-lg border p-5 shadow-md">
                      {/* Channel Pattern */}
                      <div>
                        <div className="text-foreground/60 mb-2 text-xs font-medium">
                          Channel Pattern
                        </div>
                        <div className="bg-card flex items-center justify-center rounded border px-3 py-1.5">
                          <div className="font-mono text-xs">
                            <span className="text-foreground/70">chat.</span>
                            <span className="text-blue-600 dark:text-blue-400">{'{'}</span>
                            <span className="mx-0.5 rounded bg-blue-100 px-1 text-blue-600 dark:bg-blue-900/60 dark:text-blue-400">
                              roomId
                            </span>
                            <span className="text-blue-600 dark:text-blue-400">{'}'}</span>
                      ...
                      {/* Simple divider */}
                      <div className="bg-border/30 my-3 h-px w-full"></div>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-3 gap-1.5">
                          {['lobby', 'team-1', 'team-2'].map((room) => (
                            <div
                              key={room}
                              className="bg-card/80 flex items-center justify-center rounded border px-2 py-1"
                            >
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 gap-1.5">
                          {['lobby', 'team-1', 'team-2'].map((room) => (
                            <div
                              key={room}
                              className="flex items-center justify-between rounded border border-blue-100 bg-blue-50/50 px-3 py-1.5 dark:border-blue-900/30 dark:bg-blue-950/20"
                            >
                              <div className="font-mono text-xs">
                                <span className="text-foreground/70">chat.</span>
                              ...
                              <div className="text-foreground/40 text-xs">Isolated</div>
                            </div>
                          ))}
                        </div>
                      </div>
                ...
                <div className="order-1 lg:order-2">
                  <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-blue-500/10 px-3 py-1 text-sm font-medium text-blue-500">
                    <Globe className="h-4 w-4" strokeWidth={2} />
                    <span>Dynamic Channels</span>
                  </div>
                  ...
                  <p className="text-foreground/70 mb-8 text-lg leading-relaxed">
                    Create dynamic, scoped channels with path parameters. Perfect for multi-tenant
                    apps and user-specific streams.
                  </p>

                  <ul className="space-y-4">
                    <li className="flex items-start gap-3">
                      <Check className="text-primary mt-1 h-4 shrink-0" strokeWidth={2} />
                      <span className="text-foreground/80">
            ...
            {/* Rich Presence Feature - Refined visual design */}
            <div className="mb-24 md:mb-36 lg:mb-40">
              <div className="grid grid-cols-1 items-center gap-16 lg:grid-cols-2">
                <div>
                  <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-purple-500/10 px-3 py-1 text-sm font-medium text-purple-500">
                  ...
                  <p className="text-foreground/70 mb-8 text-lg leading-relaxed">
                    Track who's online, typing status, cursor positions, and more with just a few
                    lines of code. Create truly collaborative experiences.
                  </p>
                ...
                <div className="relative">
                  <div className="to-primary/5 absolute -inset-px rounded-lg bg-gradient-to-r from-purple-500/10 opacity-50 blur-md"></div>
                  <div className="bg-card/90 relative rounded-lg border shadow-md">
                    <div className="flex items-center justify-between border-b px-5 py-3">
                      <span className="text-sm font-medium">Document.tsx</span>
                      <div className="flex items-center gap-2">
                        <div className="flex -space-x-2">
                          <Avatar className="border-background ring-primary h-6 w-6 border-2 ring-1">
                            <AvatarImage src="https://avatar.vercel.sh/alex.png" alt="Alex" />
                            <AvatarFallback>A</AvatarFallback>
                          </Avatar>
                          <Avatar className="border-background h-6 w-6 border-2">
                            <AvatarImage src="https://avatar.vercel.sh/taylor.png" alt="Taylor" />
                            <AvatarFallback>T</AvatarFallback>
                          </Avatar>
                        </div>
                        <span className="text-foreground/60 text-xs">2 editing</span>
                      </div>
                    </div>
                    ...
                    <div className="relative p-5">
                      ...
                      >
                        <MousePointer2 className="h-4 w-4" fill="rgba(59, 130, 246, 0.5)" />
                        <span className="ml-1 rounded bg-blue-500 px-1.5 py-0.5 text-xs text-white">
                          Taylor
                        </span>
                      </MotionDiv>

                      <div className="bg-secondary/20 overflow-x-auto rounded p-4 font-mono text-sm">
                        <CodeBlock code={presenceCode} language="typescript" theme="dark" />
                      </div>

                      <div className="mt-4 text-center">
                        <span className="text-foreground/60 bg-card inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-sm">
            ...
            {/* Message History Feature - Refined visual design */}
            <div className="mb-24 md:mb-36 lg:mb-40">
              <div className="grid grid-cols-1 items-center gap-16 lg:grid-cols-2">
                <div className="order-2 lg:order-1">
                  <div className="relative">
                    <div className="to-primary/5 absolute -inset-px rounded-lg bg-gradient-to-r from-amber-500/10 opacity-50 blur-md"></div>
                    <div className="bg-card/90 relative rounded-lg border p-6 shadow-md">
                      <div className="mb-6 flex items-center justify-between">
                        <span className="font-medium">Message History</span>
                        <span className="bg-secondary/60 text-foreground/60 rounded-full px-2.5 py-0.5 text-xs">
                      ...
                      <div className="space-y-6">
                        <div className="flex items-start gap-3">
                          <div className="flex flex-col items-center">
                            ...
                            <div className="bg-border/40 mt-2 h-14 w-px"></div>
                          </div>
                          <div className="bg-secondary/40 flex-1 rounded-lg p-3 text-sm">
                            <div className="mb-1 font-medium">Alex</div>
                            <div>Has anyone tested the API?</div>
                            <div className="text-foreground/50 mt-1 text-xs">10:42 AM</div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="bg-primary/10 flex-1 rounded-lg p-3 text-sm">
                            <div className="mb-1 font-medium">Taylor</div>
                            <div>Yes, it's working well!</div>
                            <div className="text-foreground/50 mt-1 text-xs">10:45 AM</div>
                          </div>
                        </div>
                        ...
                        >
                          <div className="flex flex-col items-center">
                            ...
                            <div className="bg-border/40 mt-2 h-10 w-px"></div>
                          </div>
                          <div className="flex-1 rounded-lg border border-blue-500/20 bg-blue-500/5 p-3 text-sm">
                            <div className="mb-1 font-medium">Robin</div>
                            <div>Just joined and can see all previous messages!</div>
                            <div className="text-foreground/50 mt-1 text-xs">Just now</div>
                          </div>
                        </MotionDiv>
                      </div>

                      <div className="bg-secondary/20 text-foreground/60 mt-8 rounded px-3 py-2 text-center text-xs">
                ...
                <div className="order-1 lg:order-2">
                  <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-amber-500/10 px-3 py-1 text-sm font-medium text-amber-500">
                  ...
                  <p className="text-foreground/70 mb-8 text-lg leading-relaxed">
                    Access recent messages on channel join with our built-in history capability. No
                    need to implement separate storage or manage message caching.
                  </p>
                  ...
                  <ul className="space-y-4">
                    ...
                    <li className="flex items-start gap-3">
                      <Check className="text-primary mt-1 h-4 shrink-0" strokeWidth={2} />
                      <span className="text-foreground/80">Configurable retention per channel</span>
                    </li>
                  </ul>

                  <div className="bg-secondary/30 mt-8 overflow-x-auto rounded p-4 font-mono text-sm">
                    <CodeBlock code={historyCode} language="typescript" theme="dark" />
                  </div>
                </div>
              </div>
            </div>

            {/* Authentication Feature - Refined visual design */}
            <div className="mb-24 md:mb-36 lg:mb-40">
              <div className="grid grid-cols-1 items-center gap-16 lg:grid-cols-2">
                <div>
                  <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-emerald-500/10 px-3 py-1 text-sm font-medium text-emerald-500">
                  ...
                  <p className="text-foreground/70 mb-8 text-lg leading-relaxed">
                    Integrate with your existing auth system. Grant access to specific channels
                    based on user identity and permissions with fine-grained control.
                  </p>
                ...
                <div className="relative">
                  <div className="to-primary/5 absolute -inset-1 rounded-lg bg-gradient-to-r from-emerald-500/10 opacity-50 blur-md"></div>
                  <div className="bg-card/90 relative overflow-hidden rounded-lg border shadow-md">
                    <div className="grid grid-cols-1">
                      ...
                      <div className="bg-card/80 p-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              <div className="text-sm font-medium">Your Auth System</div>
                              <div className="text-foreground/60 text-xs">
                                Auth.js, Clerk, Better-Auth, etc.
                              </div>
                            </div>
                          </div>
                          <div className="bg-secondary/30 text-foreground/60 rounded-full px-2 py-0.5 text-xs">
                            Provider
                          </div>
                        </div>
                      </div>
                      ...
                      {/* Arrow */}
                      <div className="bg-card/80 flex h-6 items-center justify-center">
                        <svg
                          width="16"
                          height="16"
                          viewBox="0 0 24 24"
                      ...
                      <div className="bg-card/80 relative p-4">
                        <div className="absolute bottom-0 left-0 top-0 w-1 bg-emerald-500/30"></div>
                        <div className="pl-2">
                          <div className="mb-2 flex items-center justify-between">
                            <div className="flex items-center gap-2.5">
                              ...
                              <div>
                                <div className="text-sm font-medium">[removed] Authorization</div>
                                <div className="text-foreground/60 text-xs">
                                  Channel access control
                                </div>
                              </div>
                            </div>
                            <div className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-xs text-emerald-500">
                              Gateway
                            </div>
                          </div>

                          <div className="bg-secondary/30 overflow-hidden rounded font-mono text-xs">
                            <div className="bg-secondary/50 text-foreground/70 border-border/30 border-b px-3 py-1 text-[10px]">
                      ...
                      {/* Channel Access Results */}
                      <div className="bg-card/80 p-4">
                        <div className="grid grid-cols-2 gap-2">
                          <div className="rounded border border-emerald-500/20 bg-emerald-500/5 p-2">
                            <div className="flex items-center justify-between">
                              <div className="text-xs font-medium">User Channel</div>
                              <div className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-1.5 py-0.5 text-[10px] text-emerald-500">
                          ...
                          <div className="rounded border border-red-500/20 bg-red-500/5 p-2">
                            <div className="flex items-center justify-between">
                              <div className="text-xs font-medium">Admin Channel</div>
                              <div className="inline-flex items-center gap-1 rounded-full bg-red-500/10 px-1.5 py-0.5 text-[10px] text-red-500">
            ...
            {/* Performance Feature - Refined visual design */}
            <div>
              <div className="grid grid-cols-1 items-center gap-16 lg:grid-cols-2">
                <div className="order-2 lg:order-1">
                  <div className="relative mx-auto max-w-md">
                    <div className="to-primary/5 absolute -inset-px rounded-lg bg-gradient-to-br from-orange-500/10 opacity-50 blur-md"></div>
                    <div className="bg-card/90 relative h-full overflow-hidden rounded-lg border shadow-md">
                      {/* Header */}
                      <div className="flex items-center justify-between border-b p-4">
                        <h4 className="text-sm font-medium">Global Edge Network</h4>
                      ...
                      {/* Map Display */}
                      <div className="p-4">
                        <div className="bg-background/30 relative aspect-[16/9] w-full overflow-hidden rounded-lg">
                          {/* World Map - Simple outlines for continents */}
                          {/* svg removed to make it easier for agent to parse file */}
                          <svg
                          ...
                          {/* Edge Locations Markers */}
                          {[
                            { x: 215, y: 150 }, // North America
                            { x: 260, y: 190 },
                            { x: 500, y: 140 }, // Europe
                            { x: 530, y: 170 },
                            { x: 700, y: 200 }, // Asia
                            { x: 750, y: 220 },
                            { x: 400, y: 320 }, // South America
                            { x: 600, y: 350 }, // Australia
                          ].map((point, i) => (
                            <MotionDiv
                              key={i}
                              ...
                              style={{
                                backgroundColor:
                                  i % 2 === 0 ? 'hsl(var(--primary))' : 'rgb(var(--orange-500))',
                                left: `${(point.x / 1000) * 100}%`,
                                top: `${(point.y / 500) * 100}%`,
                                boxShadow: '0 0 0 4px rgba(var(--primary), 0.15)',
                          ...
                          <svg className="absolute inset-0 h-full w-full" viewBox="0 0 1000 500">
                            {[
                              { from: { x: 215, y: 150 }, to: { x: 500, y: 140 } },
                              { from: { x: 500, y: 140 }, to: { x: 700, y: 200 } },
                              { from: { x: 700, y: 200 }, to: { x: 600, y: 350 } },
                              { from: { x: 215, y: 150 }, to: { x: 400, y: 320 } },
                              { from: { x: 260, y: 190 }, to: { x: 530, y: 170 } },
                              { from: { x: 530, y: 170 }, to: { x: 750, y: 220 } },
                              { from: { x: 400, y: 320 }, to: { x: 600, y: 350 } },
                            ].map((line, i) => (
                              <g key={i}>
                                {/* Base connection path */}
                                <path
                                  d={`M${line.from.x},${line.from.y} Q${(line.from.x + line.to.x) / 2},${Math.min(line.from.y, line.to.y) - 30} ${line.to.x},${line.to.y}`}
                                  stroke="rgba(59, 130, 246, 0.1)"
                                  strokeWidth="2"
                                  fill="none"
                                />

                                {/* Flowing Traffic Animation */}
                                ...
                                {/* Animated pulse effect along the path */}
                                <MotionPath
                                  d={`M${line.from.x},${line.from.y} Q${(line.from.x + line.to.x) / 2},${Math.min(line.from.y, line.to.y) - 30} ${line.to.x},${line.to.y}`}
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-3">
                          <div className="bg-background/40 rounded-lg border p-3">
                            ...
                            <div className="bg-background/50 mt-2 h-2 w-full overflow-hidden rounded-full">
                              <MotionDiv
                      ...
                      <div className="border-t p-4">
                        <div className="flex items-center">
                          <div className="mr-3 flex h-8 w-8 items-center justify-center rounded-md bg-orange-500/10">
                ...
                <div className="order-1 lg:order-2">
                  <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-orange-500/10 px-3 py-1 text-sm font-medium text-orange-500">
                  ...
                  <p className="text-foreground/70 mb-8 text-lg leading-relaxed">
                    Powered by Cloudflare Workers with Durable Objects and WebSocket Hibernation for
                    incredible performance at any scale, anywhere in the world.
                  </p>
        ...
        {/* CTA Section - More focused */}
        <section className="bg-card/30 border-border/10 border-t py-20 md:py-32">
          <div className="mx-auto max-w-5xl px-6 sm:px-8">
            <div className="border-primary/20 from-primary/5 to-background/5 rounded-xl border bg-gradient-to-b p-8 shadow-lg sm:p-12">
              <div className="grid grid-cols-1 gap-10 lg:grid-cols-5">
                <div className="lg:col-span-3">
                  <h2 className="text-3xl font-semibold tracking-tight">
                    Ready to redefine real-time?
                  </h2>
                  <p className="text-foreground/70 mt-4 max-w-xl text-lg">
                    Join the waitlist for early access and be the first to build with [removed].
                  </p>
                ...
                <div className="lg:col-span-2">
                  <div className="bg-card h-full rounded-lg p-5 shadow-sm">
                    <h4 className="flex items-center gap-2 font-medium">
                      <Check className="text-primary h-5 w-5" strokeWidth={2} />
                      <span>Early Access Benefits</span>
                    </h4>
                    <ul className="mt-4 space-y-3">
                      <li className="flex items-start gap-2">
                        <Check className="text-primary mt-0.5 h-4 w-4" strokeWidth={2} />
                        <span className="text-foreground/80 text-sm">
                          Early access to the [removed] platform
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <Check className="text-primary mt-0.5 h-4 w-4" strokeWidth={2} />
        ...
        {/* Simple Footer */}
        <footer className="bg-background border-border/20 border-t py-10">
          <div className="mx-auto max-w-7xl px-6 sm:px-8">
            <div className="flex flex-col items-center gap-6 md:flex-row md:justify-between">
              <div className="flex flex-col items-center gap-2 md:items-start">
                <div className="text-foreground text-xl font-semibold tracking-tight">[removed]</div>
                <p className="text-foreground/60 text-center text-sm md:text-left">
                  Type-safe real-time for modern applications.
                </p>
              </div>

              <div className="flex items-center gap-6">
                <a
                  href="https://x.com/use[removed]"
            ...
            <div className="border-border/10 mt-6 border-t pt-6 text-center">
