...
>(
  ...
  const state = useRef(
    proxy({
      ...
      connectionState: use[removed]Store.getState().connectionState,
  ...
  useEffect(() => {
    ...
    const unsub = use[removed]Store.subscribe((newState, prevState) => {
    ...
    const currentConnState = use[removed]Store.getState().connectionState;
  ...
  useEffect(() => {
    ...
    const unsubZustand = use[removed]Store.subscribe((newState, prevState) => {
