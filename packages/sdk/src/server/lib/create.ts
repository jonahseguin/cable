...
export type Sock8Container = ReturnType<typeof create>;

export function create(options: CreateOptions) {
  const { apiKey, __baseUrl, __baseWebSocketUrl } = options;
  const apiBaseUrl = __baseUrl ?? 'https://api.sock8.com';
  const baseWebSocketUrl = __baseWebSocketUrl ?? 'wss://api.sock8.com/connect';
  ...
  async function _makeApiRequest(endpoint: string, body: any, method: 'POST' | 'GET' = 'POST') {
    console.debug(
      `[API Request] Endpoint: ${apiBaseUrl}${endpoint}, Key: ${apiKey ? `sock8_******${apiKey.slice(-6)}` : 'NONE'}`,
      {
        body,
      },
    );
