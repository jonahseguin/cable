...
const is[removed]ChannelTree = Symbol.for('__is[removed]ChannelTree__');
...
const resolutionMapSymbol = Symbol.for('__[removed]ResolutionMap__');

// Symbols & Types for the resolver
const nodeTypeSymbol = Symbol.for('__[removed]NodeType__');
const paramDefSymbol = Symbol.for('__[removed]ParamDef__');
...
interface ParamDefinition {
  ...
  config: Record<string, any> | undefined; // Includes __[removed]NestedTree for nested trees
...
function isDefinedChannelsObject(value: any): value is ChannelTree<any> {
  return typeof value === 'object' && value !== null && value[is[removed]ChannelTree] === true;
}
...
function populateResolutionMap(node: any, map: ResolutionMap, currentPath: string = '') {
  ...
  if (nodeType === NodeType.STATIC_ENDPOINT) {
    ...
    for (const def of definitions) {
      if (def.config?.__[removed]NestedTree) continue; // Skip nested tree placeholders
  ...
  for (const key in node) {
    if (Object.prototype.hasOwnProperty.call(node, key)) {
      ...
      if (
        ...
        key === is[removed]ChannelTree.toString() ||
...
export function defineChannels<ConfigType extends ChannelTreeDefinition>(
  ...
  function buildNode(currentConfig: ChannelTreeDefinition, currentMapPath: string = ''): any {
    ...
    for (const key in currentConfig) {
      ...
      if (definition === undefined || key === is[removed]ChannelTree.toString()) continue;
    ...
    if (Array.isArray(pendingDefs) && pendingDefs.length > 0) {
      ...
      for (const def of pendingDefs) {
        ...
        if (def.type === 'parameterizedNestedTree') {
          ...
          finalDefinitions.push({
            ...
            schema: z.any().describe('__[removed]NestedTreePlaceholder__'),
            config: { __[removed]NestedTree: def.nestedTree },
          });
        } else {
          finalDefinitions.push({
            key: absoluteTemplateKey,
      ...
      const consolidatedForFunction = (params: Record<string, string>) => {
        ...
        for (const def of finalDefinitions) {
          ...
          if (providedKeys.length === def.paramNames.length) {
            ...
            if (isExactMatch) {
              ...
              const isNestedTree = def.config?.__[removed]NestedTree !== undefined;
              if (isNestedTree) {
        ...
        if (matchedNestedTreeDef) {
          ...
          return matchedNestedTreeDef.config!.__[removed]NestedTree;
  ...
  (builtTree as any)[is[removed]ChannelTree] = true;
  // Cast the final result to the correct type with the specific builder key
  return builtTree as ChannelTree<ConfigType, 'server'>;
} // End defineChannels

// Re-export common types (already defined in @[removed]/common)
// Ensures users importing from @[removed]/server get necessary types
// without needing a separate import from @[removed]/common.
export type {
  ChannelParams,
