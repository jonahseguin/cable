...
function safeStringify(data: unknown, fallback = '{}'): string {
  try {
    ...
    console.error('[Sock8ConnectionManager] Failed to stringify data:', e);
    return fallback;
  }
}

/**
 * Implementation of the ConnectionManager for sock8
 */
export class Sock8ConnectionManager implements ConnectionManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (
            ...
            handlers?.forEach((handler) => {
              try {
                ...
                console.error(
                  `[Sock8ConnectionManager] Error in presenceState handler for channel "${message.channel}":`,
                  error,
                );
              }
            });
          ...
          // Runtime check for presence update message
            ...
            handlers?.forEach((handler) => {
              try {
                ...
                console.error(
                  `[Sock8ConnectionManager] Error in presenceUpdate handler for channel "${message.channel}":`,
                  error,
                );
              }
            });
          ...
          console.error(
            '[Sock8ConnectionManager] Failed to parse or route message',
            error,
            'Raw data:',
            event.data,
          );
  ...
  setPresence(channel: string, data: unknown): void {
    if (this.state !== ConnectionState.CONNECTED || !this.ws) {
      console.warn('[Sock8ConnectionManager] Cannot setPresence, not connected.');
      return;
    }
    ...
    console.debug(`[Sock8ConnectionManager] Sending setPresence for ${channel}`);
  ...
  getPresence(channel: string): void {
    if (this.state !== ConnectionState.CONNECTED || !this.ws) {
      console.warn('[Sock8ConnectionManager] Cannot getPresence, not connected.');
      return;
    }
    const message = {
      event: 'getPresence',
    ...
    console.debug(`[Sock8ConnectionManager] Sending getPresence for ${channel}`);
    this.ws.send(safeStringify(message));
  }
